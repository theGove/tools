...
function updateLoginButton(): void {
    ...
    if (!(loginButton instanceof HTMLElement)) { return }
    if (globals.user) {
...
function searchBook(): void {
    fetch(`/feeds/posts/default?alt=json&label=chapter&v=2&orderby=relevance&max-results=100&q=label%3Achapter+${encodeURIComponent((tag("search") as HTMLInputElement).value)}&start-index=1&rewriteforssl=true`)
        .then(response => {
            if (!response.ok) {
...
function findLink(links: FeedLink[]) {
...
function buildChapterSearchResult(entry: FeedEntry): void {
    //console.log(entry)
    ...
    const searchTerm = (tag("search") as HTMLInputElement).value
...
function entryHasLabel(entry: FeedEntry, label: string): boolean {
...
function tag(id: string): HTMLElement | null {
...
function showSection(section: number | string, recordHash = true): void {
    ...
    if (Number.isNaN(Number(section))) {
        // section is string and should be 'next' or 'prior'
        ...
        // section numeric (number or numeric string — keep original value)
        sectionToShow = section
    }

    // prevents sectionToShow from being out of bounds
    if (isNaN(Number(sectionToShow))) { return }
...
function navigate(direction: string): void {
    ...
    const targetNode = tag(direction + "-button")!
...
function courseRoleLabel(course: Pick<Course, "role" | "roles">): string {
    ...
    const visible = roles.filter(
        (role: string) => typeof role === "string" && role.trim() && role.trim().toLowerCase() !== "learner"
    )
 ...
 */
function bookSlugFromHost(hostOrUrl: string | null | undefined): string | null {
 ...
 */
function currentBookSlug(): string | null {
...
function firstCourseWithBook(courses: Course[], bookSlug: string): Course | null {
    return (
        courses.find((course: Course) => courseBookSlugs(course).includes(bookSlug)) ||
        null
    )
}
 ...
 */
function bookUrl(slug: string, position?: { chapter?: string; section?: string; path?: string } | null) {
 ...
 */
function bookSectionHash(section: string | undefined): string {
...
function chapterSectionElements(): HTMLElement[] {
    ...
    return [...document.querySelectorAll(".chapter-section")].filter(
        (el): el is HTMLElement => el instanceof HTMLElement && !!el.id && el.style.display !== "none",
    )
}
 ...
 */
function courseBookSlugs(course: Pick<Course, "book">): string[] {
 ...
 */
function renderCourseBookButtons(course: Course, isCurrentCourse: boolean): string {
 ...
 */
async function updateMenuCourses(): Promise<void> {
    const container = tag("menu-courses")
    if (!container) {
        return
    }
 ...
 * @param {boolean} isCurrentCourse - Whether this course is the session org.
 */
function renderMenuCourseItem(course: Course, isCurrentCourse: boolean): string {
...
function renderMenuCourses(container: HTMLElement, courses: Course[]): void {
    ...
    const others = courses.filter((course: Course) => course.workosOrganizationId !== organizationId)
 ...
 */
async function selectMenuCourse(organizationId: string, trigger: HTMLElement, bookSlug: string | null): Promise<void> {
...
function getChaptSections(obj: BookSectionNode, html: string[]): void {
    ...
    function lastId(id: string): string {
...
const showToast: ShowToastFn = function showToast(message, kind) {
...
function closeMessage(evt?: Event): void {
...
function message({ text: messageHtml = "An error occurred.", title: titleText = "System Message", buttons: callbacks = [{ text: "OK", fn: closeMessage }], seconds: secondsUntilClose, type = "info", modal = false }: MessageOptions = {}): void {
    ...
    const dialog = document.createElement("div") as MsgDialogElement
...
function fontSize(adjustment?: number): void {
    ...
    if (!adjustment) {
        ...
        globals.variables.fontZoom = Math.round(((globals.variables.fontZoom ?? 1) + adjustment) * 10) / 10
    ...
    fetch(audioDataUrl)
        ...
        .then(html => {
            ...
            audioEl.addEventListener('ended', () => {
                ...
                let audioTag: HTMLElement | null = tag(elem.dataset.next || "")
                if (!audioTag) { return }
 ...
 * @param {string} organizationId - WorkOS organization id.
 */
async function fetchBookActivity(organizationId: string): Promise<BookActivity | null> {
 ...
 */
async function putBookActivity(position: { book: string; chapter: string; section: string; path?: string }): Promise<BookActivity | null> {
 ...
 */
function scheduleBookActivitySave(section?: string): void {
 ...
 */
function currentlyVisibleSection(): number | null {
 ...
 */
async function syncBookActivityForActiveCourse(): Promise<void> {
    globals.trackBookActivity = false
    globals.bookActivity = null
